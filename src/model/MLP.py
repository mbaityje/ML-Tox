...
import pandas as pd
import numpy as np
from helpers.helper_mlp import (
    build_models,
    build_hypermodel,
    build_hypermodel_multiclass,
    prepare_data,
)
...
cv_fixed_models = (
    True  # fit models with fixed hyperparamters and perform cross-validation?
)
...
X_trainval, y_trainval, X_test, y_test, dummy = prepare_data(
    path_data, setup, numerical, multiclass=multiclass
)
...
if cv_fixed_models:
    ...
    for train_index, val_index in kf.split(X=X_trainval, y=y_trainval):
        ...
        if logreg_keras:
            ...
            pd.DataFrame.from_dict(hist.history).to_csv(
                fldr_curr + "model0_training_hist_cv" + str(cv) + ".txt", index=False
            )
        ...
        if fit_tuned_model:
            ...
            pd.DataFrame.from_dict(hist.history).to_csv(
                fldr_curr + "model_tuned_training_hist_cv" + str(cv) + ".txt",
                index=False,
            )
            model.save(fldr_curr + "model_tuned_cv" + str(cv) + ".h5")
            dsp.plot_history(
                hist, file=fldr_curr + "model_tuned_training_hist" + str(cv) + ".pdf"
            )
...
for trn_ind, val_ind in kfold.split(np.zeros(X_trainval.shape[0]), y_trainval):
    ...
    if fit_base_model:
        ...
        pd.DataFrame.from_dict(hist.history).to_csv(
            casepath + "model_base_training_hist.txt", index=False
        )
