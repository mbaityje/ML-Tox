...
def load_raw_data(DATA_PATH_TESTS, DATA_PATH_RESULTS, DATA_PATH_SPECIES,
    ...
    for i in DATA_PROPERTY_PATH:
        df_property = pd.concat([
            df_property,
            pd.read_excel(open(i, 'rb'),
                          usecols=[
                              'Substance_CASRN', 'Structure_SMILES',
                              'Structure_MolWt', 'NCCT_MP', 'NCCT_WS'
                          ],
                          engine='openpyxl')
        ],
                                axis=0)
...
def impute_conc(results_prefiltered):
    ...
    keep_conc_unit = ['ppb', 'ppm', 'ug/L', 'ng/L', 'mg/L', 'ng/ml', 'mg/dm3',\
    ...
    to_drop_conc_mean = db[(db.conc1_mean == 'NC') | (db.conc1_mean == 'NR')
                           | db.conc1_mean.isnull()].index
    ...
    to_drop_invalid_conc = db_filtered_mean[
        (db_filtered_mean.conc1_mean == '>100000') |
...
def select_impute_features(prefiltered_results):
    keep_columns = [
        'obs_duration_mean', 'obs_duration_unit', 'endpoint', 'effect',
        'measurement', 'conc1_type', 'conc1_mean', 'conc1_unit', 'test_cas',
        'test_location', 'exposure_type', 'control_type', 'media_type',
        'application_freq_unit', 'class', 'tax_order', 'family', 'genus',
        'species', 'smiles', 'mol_weight', 'melting_point', 'water_solubility'
    ]
...
def repeated_experiments(imputed_db):
    ...
    final_db = db.groupby(by=[
        'test_cas', 'smiles', 'obs_duration_mean', 'conc1_type', 'fish',
...
def process_features(chemical_features):
    ...
    db[['water_solubility'
        ]] = pt.transform(db.water_solubility.values.reshape(-1, 1)).ravel()
    ...
    for i in (range(pubchem.shape[0])):
        try:
            pubchem.loc[i, "pubchem2d"] = pcp.get_compounds(
                pubchem["smiles"][i], 'smiles')[0].cactvs_fingerprint
        except:
